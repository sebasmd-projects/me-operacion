/* =====================================================================
   logica-hlr-hss-tigo.js · Motor "Tigo" de la pestaña HLR/HSS
   ---------------------------------------------------------------------
   Copia envuelta de logica-qdn-tigo.js (Consulta QDN · Tigo), SIN tocar
   una sola linea de sus reglas de negocio. El envoltorio (IIFE + exponer
   iniciar()/reiniciar()/filasExport()/filasParaExportar()) es lo unico
   nuevo: existe porque esta pagina monta varios motores a la vez y
   todos declaran los mismos nombres de nivel superior (filas, render,
   consultar, ejecutarPool...). Sin el IIFE chocarian; sin exponer estas
   dos funciones, el puente (que vive FUERA del IIFE) no podria armar
   los botones de exportacion CSV/Excel/JSON.

   Si cambia una regla de Tigo, revisa tambien si aplica a
   logica-qdn-tigo.js / logica-portabilidad-tigo.js (motor hermano).
===================================================================== */
(function () {
"use strict";

/* =====================================================================
   logica-qdn-tigo.js · Consulta QDN Tigo (HLR/HSS)
   ---------------------------------------------------------------------
   Consulta por MSISDN el estado de la línea en el HLR de Tigo (servicio
   de autogestión, el mismo que usaba CheckPortabilidad.html), como
   equivalente al validador QDN de Claro: es una consulta neutral de
   estado, sin framing de portabilidad. Trae:
...
--------------------------------------------------------------------- */
const RETCODES = {
    // 0     -> el subscriptor tiene perfil creado en el HLR/HSS.
    // 3001  -> "Subscriber not defined": la línea NO tiene perfil en el HLR.
    //          Por sí solo NO dice a quién pertenece el número; caben tres
    //          lecturas y hay que cruzarlas con el estado en ME (Optiva):
    //            · activa en ME pero sin perfil -> inconsistencia, se escala
    //              a Optiva;
    //            · inactiva en ME -> sin perfil es lo esperado, no se escala;
    //            · el número no es de Móvil Éxito (puede ser un Port Out).
    // 1033  -> "Number threshold exceeded": la línea era de Tigo y se portó a
    //          Móvil Éxito, pero Tigo no la eliminó de su HLR/HSS. Queda un
    //          residuo que impide crear el perfil: se escala a Tigo y, una
    //          vez depurado, la consulta pasa a responder 3001.
    "0": { estado: "ACTIVA", etiqueta: "Activa" },
    "3001": { estado: "SIN_PERFIL", etiqueta: "Sin perfil" },
    "1033": { estado: "RESIDUO_TIGO", etiqueta: "Residuo en Tigo" }
};
...
function procesarRespuestaTigo(msisdn, cuerpo, meta) {
    ...
    /* ---- coherencia: la respuesta debe hablar de la línea consultada -----
       ...
       (o un stub de QA) haría que TODAS las líneas salgan como "Activa".
...
const columnas = ["msisdn", "estado", "retcode", "imsi", "imei", "card", "bloqueos", "roaming", "desvios", "detalle"];
const ETIQUETA_COLUMNA = {
    msisdn: "MSISDN", estado: "Estado", retcode: "RETCODE", imsi: "IMSI", imei: "IMEI",
...
function actualizarProgreso(hechas, total) {
    ...
    const c = { ACTIVA: 0, SIN_PERFIL: 0, RESIDUO_TIGO: 0, TIMEOUT: 0, ERROR: 0, ERROR_RETCODE: 0, INVALID: 0, INCONSISTENTE: 0 };
    filas.forEach(f => { if (c[f.estadoProceso] !== undefined) c[f.estadoProceso]++; });
    MEUI.$("#progresoTexto").textContent =
        `Procesando: ${hechas} / ${total}  ·  Activas ${c.ACTIVA}  ·  Sin perfil ${c.SIN_PERFIL}  ·  ` +
...
async function consultar() {
    ...
    filas = entrada.lineas.map(m => ({ msisdn: m, estadoProceso: "EN_COLA", resumen: {} }));
    ...
    const c = { ACTIVA: 0, SIN_PERFIL: 0, RESIDUO_TIGO: 0, TIMEOUT: 0, ERROR: 0, ERROR_RETCODE: 0, INVALID: 0, INCONSISTENTE: 0 };
    filas.forEach(f => { if (c[f.estadoProceso] !== undefined) c[f.estadoProceso]++; });
    MEUI.log(`Resultado — Total: ${filas.length}  ·  Activas: ${c.ACTIVA}  ·  Sin perfil: ${c.SIN_PERFIL}  ·  ` +
...
function actualizarKPIs() {
    const c = { ACTIVA: 0, SIN_PERFIL: 0, RESIDUO_TIGO: 0, TIMEOUT: 0, ERROR: 0, ERROR_RETCODE: 0, INVALID: 0, INCONSISTENTE: 0 };
    filas.forEach(f => { if (c[f.estadoProceso] !== undefined) c[f.estadoProceso]++; });
    MEUI.$("#kpiActivas").textContent = c.ACTIVA;
...
function celdaEstado(f) {
    ...
    const cls = { ACTIVA: "ok", SIN_PERFIL: "off", RESIDUO_TIGO: "warn", TIMEOUT: "warn", INCONSISTENTE: "warn",
...
function construirDataTable() {
    dataTable = new DataTable("#tablaResultados", MEUI.opcionesTabla({
        ...
        columns: [
            ...
            {
                data: null, title: "Bloqueos", render: f => f.estadoProceso === "ACTIVA"
...
function abrirDetalle(msisdn) {
    ...
    const cls = { ACTIVA: "ok", SIN_PERFIL: "off", RESIDUO_TIGO: "warn", ERROR_RETCODE: "err", INCONSISTENTE: "warn" }[f.estadoProceso] || "neutro";
...
/* ---------------------------------------------------------------------
   14 · EXPORTACIÓN
--------------------------------------------------------------------- */
const CABECERA_EXPORT = ["MSISDN", "Estado", "RETCODE", "Mensaje", "IMSI", "IMEI", "Tipo SIM",
...
function filasExport() {
    const rows = filas.filter(filaPasaFiltros).map(f => [
        f.msisdn, ETIQUETA_ESTADO[f.estadoProceso] || f.estadoProceso, f.retcode || "", f.retmsg || f.detalle || "",
...
function filasParaExportar() {
    return filas.filter(filaPasaFiltros).map(f => ({
        msisdn: f.msisdn, estado: ETIQUETA_ESTADO[f.estadoProceso] || f.estadoProceso,
...
// Diferido hasta que la pestaña "Tigo" se monte de verdad (ver
// me-hlr-hss-puente.js). Si se ejecutara al cargar el script,
// MEUI.$("#tablaResultados") etc. no encontrarian nada todavia.
window.MotorTigo = {
    iniciar: inicializarTigo,
    // Al volver a esta pestaña, dataTable sigue apuntando a la tabla
    // VIEJA (ya desmontada del DOM). Se limpia antes de reiniciar para
    // que construirDataTable() cree una nueva sobre el <table> fresco.
